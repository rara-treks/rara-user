  ...
  ({ className, variant = "outline", size = "icon", ...props }, ref) => {
    ...
    return (
      <Button
        ...
        size={size}
        className={cn(
          "absolute carousel-nav-btn h-8 w-8 rounded-full",
...
const CarouselDots = React.forwardRef<HTMLButtonElement, React.ComponentPropsWithRef<"button">>(
  ({ className, ...props }, ref) => {
    const { api } = useCarousel();
    const { selectedIndex, scrollSnaps, onDotButtonClick } = useDotButton(api);

    return (
      <div className="flex gap-1 items-center absolute bottom-2 left-1/2 -translate-x-1/2">
        {scrollSnaps.length > 1 &&
          scrollSnaps.map((_, index) => (
            <DotButton
              ...
              style={{
                width: selectedIndex === index ? 8 : 8 - index * 0.7,
                height: selectedIndex === index ? 8 : 8 - index * 0.7,
